...
use super::{
    ...
    types::{
        AdminProduct, AdminProductConnection, AdminProductVariant, BalanceTransaction,
        BalanceTransactionConnection, BalanceTransactionSourceType, BalanceTransactionType,
        BankAccount, BankAccountStatus, CalculatedOrder, Collection, CollectionConnection,
        CollectionProduct, CollectionWithProducts, Customer, CustomerConnection, CustomerSegment,
        DiscountCode, DiscountCodeConnection, DiscountCombinesWith, DiscountListConnection,
        DiscountListItem, DiscountMethod, DiscountMinimumRequirement, DiscountSortKey,
        DiscountStatus, DiscountType, DiscountValue, Dispute, DisputeAddress, DisputeConnection,
        DisputeDetail, DisputeEvidence, DisputeFileUpload, DisputeFulfillment,
        DisputeReasonDetails, DisputeStatus, DisputeType, FulfillmentHoldInput,
        FulfillmentHoldReason, FulfillmentOrder, FulfillmentOrderLineItem, GiftCard,
        GiftCardConfiguration, GiftCardConnection, GiftCardDetail, GiftCardRecipient,
        ...
        OrderEditUpdateShippingLineInput, PageInfo, Payout, PayoutConnection, PayoutDetail,
        PayoutSchedule, PayoutScheduleInterval, PayoutStatus, PayoutSummary, PayoutTransactionType,
...
use queries::{
    ...
    DisputeEvidenceUpdate, FileDelete, FileUpdate, FulfillmentCreate, FulfillmentOrderHold,
    FulfillmentOrderReleaseHold, FulfillmentTrackingInfoUpdate, GetBankAccounts, GetCollection,
    GetCollectionWithProducts, GetCollections, GetCustomer, GetCustomerSegments, GetCustomers,
    GetDiscountCode, GetDiscountCodes, GetDiscountNodes, GetDispute, GetDisputes,
    GetFulfillmentOrders, GetGiftCardConfiguration, GetGiftCardDetail, GetGiftCards,
    GetGiftCardsCount, GetInventoryItem, GetInventoryItems, GetInventoryLevels, GetLocations,
    GetOrder, GetOrderDetail, GetOrders, GetPayout, GetPayoutDetail, GetPayoutSchedule,
    GetPayoutTransactions, GetPayouts, GetProduct, GetProducts, GetPublications, GiftCardCreate,
    GiftCardCredit, GiftCardDeactivate, GiftCardDebit, GiftCardSendNotificationToCustomer,
    GiftCardSendNotificationToRecipient, GiftCardUpdate, InventoryAdjustQuantities,
    InventorySetQuantities, MoveInventory, OrderCancel, OrderCapture, OrderClose,
...
impl AdminClient {
    ...
    pub async fn get_payouts(
        ...
        first: i64,
        after: Option<String>,
        sort_key: Option<super::types::PayoutSortKey>,
        reverse: bool,
    ) -> Result<PayoutConnection, AdminShopifyError> {
        use super::types::PayoutSortKey;
        use queries::get_payouts::PayoutSortKeys;
    ...
    pub async fn get_payout_detail(&self, id: &str) -> Result<PayoutDetail, AdminShopifyError> {
        let variables = queries::get_payout_detail::Variables { id: id.to_string() };
        ...
        use queries::get_payout_detail::GetPayoutDetailNode;
        match node {
            GetPayoutDetailNode::ShopifyPaymentsPayout(p) => {
                ...
                // Note: gross is deprecated but we use it for display purposes
                #[allow(deprecated)]
                let gross = Money {
    ...
    pub async fn get_payout_transactions(
        ...
        // Build query filter using payout_date if provided
        // The payout_date filter expects YYYY-MM-DD format, but issued_at is ISO 8601
        // Extract just the date portion if it contains a 'T' (datetime format)
        ...
        let variables = queries::get_payout_transactions::Variables {
            first: Some(first),
            after,
            query,
        };
        ...
        // Log the first few transaction payout IDs to debug filtering
        ...
        let transactions: Vec<BalanceTransaction> = account
            ...
            .filter_map(|e| {
                ...
                // Filter by payout_id if provided
                if let Some(ref filter_payout_id) = payout_id {
                    // associated_payout.id may be None for some transaction types
    ...
    pub async fn get_disputes(
        ...
        let variables = queries::get_disputes::Variables {
            first: Some(first),
            after,
            query,
        };
    ...
    pub async fn get_dispute(&self, id: &str) -> Result<DisputeDetail, AdminShopifyError> {
        let variables = queries::get_dispute::Variables { id: id.to_string() };
        ...
        use queries::get_dispute::GetDisputeNode;
        match node {
    ...
    pub async fn get_bank_accounts(&self) -> Result<Vec<BankAccount>, AdminShopifyError> {
        let variables = queries::get_bank_accounts::Variables { first: Some(10) };
    ...
    pub async fn get_payout_schedule(&self) -> Result<PayoutSchedule, AdminShopifyError> {
        let variables = queries::get_payout_schedule::Variables {};
...
const fn convert_payout_status_detail(
    status: &queries::get_payout_detail::ShopifyPaymentsPayoutStatus,
) -> PayoutStatus {
    match status {
        queries::get_payout_detail::ShopifyPaymentsPayoutStatus::SCHEDULED
        | queries::get_payout_detail::ShopifyPaymentsPayoutStatus::Other(_) => {
            PayoutStatus::Scheduled
        }
        queries::get_payout_detail::ShopifyPaymentsPayoutStatus::IN_TRANSIT => {
            PayoutStatus::InTransit
        }
        queries::get_payout_detail::ShopifyPaymentsPayoutStatus::PAID => PayoutStatus::Paid,
        queries::get_payout_detail::ShopifyPaymentsPayoutStatus::FAILED => PayoutStatus::Failed,
        queries::get_payout_detail::ShopifyPaymentsPayoutStatus::CANCELED => PayoutStatus::Canceled,
    }
}

/// Convert GraphQL payout transaction type to domain type.
const fn convert_payout_transaction_type(
    tx_type: &queries::get_payout_detail::ShopifyPaymentsPayoutTransactionType,
) -> PayoutTransactionType {
    match tx_type {
        queries::get_payout_detail::ShopifyPaymentsPayoutTransactionType::DEPOSIT
        | queries::get_payout_detail::ShopifyPaymentsPayoutTransactionType::Other(_) => {
            PayoutTransactionType::Deposit
        }
        queries::get_payout_detail::ShopifyPaymentsPayoutTransactionType::WITHDRAWAL => {
...
const fn convert_transaction_type(
    tx_type: &queries::get_payout_transactions::ShopifyPaymentsTransactionType,
) -> BalanceTransactionType {
    use queries::get_payout_transactions::ShopifyPaymentsTransactionType;
    match tx_type {
        ...
        // Default to Adjustment for adjustments and any unknown/other types
...
const fn convert_source_type(
    src: Option<&queries::get_payout_transactions::ShopifyPaymentsSourceType>,
) -> BalanceTransactionSourceType {
    use queries::get_payout_transactions::ShopifyPaymentsSourceType;
    match src {
        ...
        // Unknown source type or None
        _ => BalanceTransactionSourceType::Unknown,
    }
}

/// Convert GraphQL dispute status to domain type.
const fn convert_dispute_status(status: &queries::get_disputes::DisputeStatus) -> DisputeStatus {
    match status {
        queries::get_disputes::DisputeStatus::NEEDS_RESPONSE
        | queries::get_disputes::DisputeStatus::Other(_) => DisputeStatus::NeedsResponse,
        queries::get_disputes::DisputeStatus::UNDER_REVIEW => DisputeStatus::UnderReview,
        queries::get_disputes::DisputeStatus::CHARGE_REFUNDED => DisputeStatus::ChargeRefunded,
        queries::get_disputes::DisputeStatus::ACCEPTED => DisputeStatus::Accepted,
        queries::get_disputes::DisputeStatus::WON => DisputeStatus::Won,
        queries::get_disputes::DisputeStatus::LOST => DisputeStatus::Lost,
    }
}

/// Convert GraphQL dispute status (detail query) to domain type.
const fn convert_dispute_status_detail(
    status: &queries::get_dispute::DisputeStatus,
) -> DisputeStatus {
    match status {
        queries::get_dispute::DisputeStatus::NEEDS_RESPONSE
        | queries::get_dispute::DisputeStatus::Other(_) => DisputeStatus::NeedsResponse,
        queries::get_dispute::DisputeStatus::UNDER_REVIEW => DisputeStatus::UnderReview,
        queries::get_dispute::DisputeStatus::CHARGE_REFUNDED => DisputeStatus::ChargeRefunded,
        queries::get_dispute::DisputeStatus::ACCEPTED => DisputeStatus::Accepted,
        queries::get_dispute::DisputeStatus::WON => DisputeStatus::Won,
        queries::get_dispute::DisputeStatus::LOST => DisputeStatus::Lost,
    }
}

/// Convert GraphQL dispute type to domain type.
const fn convert_dispute_type(dispute_type: &queries::get_disputes::DisputeType) -> DisputeType {
    match dispute_type {
        queries::get_disputes::DisputeType::CHARGEBACK
        | queries::get_disputes::DisputeType::Other(_) => DisputeType::Chargeback,
        queries::get_disputes::DisputeType::INQUIRY => DisputeType::Inquiry,
    }
}

/// Convert GraphQL dispute type (detail query) to domain type.
const fn convert_dispute_type_detail(
    dispute_type: &queries::get_dispute::DisputeType,
) -> DisputeType {
    match dispute_type {
        queries::get_dispute::DisputeType::CHARGEBACK
        | queries::get_dispute::DisputeType::Other(_) => DisputeType::Chargeback,
        queries::get_dispute::DisputeType::INQUIRY => DisputeType::Inquiry,
    }
}

/// Convert GraphQL bank account status to domain type.
const fn convert_bank_account_status(
    status: &queries::get_bank_accounts::ShopifyPaymentsBankAccountStatus,
) -> BankAccountStatus {
    use queries::get_bank_accounts::ShopifyPaymentsBankAccountStatus;
    match status {
...
const fn convert_payout_interval(
    interval: &queries::get_payout_schedule::ShopifyPaymentsPayoutInterval,
) -> PayoutScheduleInterval {
    match interval {
        queries::get_payout_schedule::ShopifyPaymentsPayoutInterval::DAILY
        | queries::get_payout_schedule::ShopifyPaymentsPayoutInterval::Other(_) => {
            PayoutScheduleInterval::Daily
        }
        queries::get_payout_schedule::ShopifyPaymentsPayoutInterval::WEEKLY => {
            PayoutScheduleInterval::Weekly
        }
        queries::get_payout_schedule::ShopifyPaymentsPayoutInterval::MONTHLY => {
            PayoutScheduleInterval::Monthly
        }
        queries::get_payout_schedule::ShopifyPaymentsPayoutInterval::MANUAL => {
...
#[allow(deprecated)] // email field is deprecated but still needed for compatibility
fn extract_dispute_order_info(
    order: Option<&queries::get_dispute::GetDisputeNodeOnShopifyPaymentsDisputeOrder>,
) -> DisputeOrderInfo {
...
#[allow(deprecated)] // country_code is deprecated but needed for compatibility
fn convert_dispute_evidence(
    e: &queries::get_dispute::GetDisputeNodeOnShopifyPaymentsDisputeDisputeEvidence,
) -> DisputeEvidence {
    DisputeEvidence {
